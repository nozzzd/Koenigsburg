...
2. Open **SQL Editor → New query**, paste the contents of
   ...
   - **Inventory ledger:** [`supabase/011_inventory.sql`](supabase/011_inventory.sql)
     creates the sources, containers, item counts, and search functions.
     [`supabase/012_qmsync_v1.sql`](supabase/012_qmsync_v1.sql) adds atomic
     full-snapshot storage for `/api/handshake` and `/api/sync`. Configure
     `QMSYNC_SERVER_ID` (and the optional friendly name) from `.env.example`;
     the exact v1 contract is in [`QMSYNC.md`](QMSYNC.md).
